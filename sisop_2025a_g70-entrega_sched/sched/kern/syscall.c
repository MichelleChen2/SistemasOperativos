...
sys_exofork(void)
{
	...
	newenv->env_priority =
	        curenv->env_priority;  /// Resetting de este campo (hijo con prioridad mayor que la del padre)
...
// Busca el Env a partir del envid y devuelve el priority del Env en caso que lo
// encuentra y E_BAD_ENV en caso contrario
static int
...
// Busca el Env a partir del envid y decrece la prioridad del Env en 1, devuelve
// 0 en caso de éxito E_BAD_ENV en caso contrario
static int
sys_decpriority(envid_t envid, uint32_t valor_a_decrementar)
{
	struct Env *env;
	if (envid2env(envid, &env, 0) != 0)
		return -E_BAD_ENV;

	env->env_priority += valor_a_decrementar;
	return 0;
}
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	case SYS_decpriority:
		return sys_decpriority(a1, a2);
	default:
		return -E_INVAL;
	}
}
