...
int irq_count;  /// Variable para detectar cuándo se generó un IRQ
...
env_init(void)
{
	irq_count = 0;  /// Inicialización del contador global de IRQ
	// Set up envs array
	for (int i = 0; i < NENV; i++) {
		envs[i].env_id = 0;
		envs[i].env_status = ENV_FREE;
		/// Este campo será básicamente para el uso de "env_free_list"
		envs[i].env_link = (envs + i + 1);
	}
	envs[NENV - 1].env_link = NULL;
	/// Simplemente se comparte el puntero de "envs" a "env_free_list"
	env_free_list = envs;
...
env_alloc(struct Env **newenv_store, envid_t parent_id)
{
	...
	int r;
	struct Env *e;
	/// El nuevo proceso tendrá el puntero del primer elemento de la lista "env_free_list"
	if (!(e = env_free_list))
	...
	/// SIMPLEMENTE SE ESTÁ AVANZANDO EL PUNTERO de "env_free_list" debido a que se creó el nuevo proceso
	env_free_list = e->env_link;
	*newenv_store = e;  /// Se está pasando el puntero al nuevo proceso
